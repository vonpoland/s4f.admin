...
const dbConnectionMiddleware = db.dbConnectionMiddleware;
...
router.put('/:id/editable', secure, (req, res) => {
    ...
    pollService.edit.call(req.user._dbConnection, req.params.id, updateOnlyPath('editable', req.body), err => {
        if (err) {
            logger.error(err);
            return res.sendStatus(HttpStatus.BAD_REQUEST);
        }
...
router.put('/:id/data/options', secure, (req, res) => {
    // don't let user to edit other properties than editable.
    pollService.edit.call(req.user._dbConnection, req.params.id, updateOnlyPath('data.options', req.body), err => {
        if (err) {
            logger.error(err);
            return res.sendStatus(HttpStatus.BAD_REQUEST);
        }
